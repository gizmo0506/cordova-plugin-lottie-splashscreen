...
This fork adds Swift Package Manager support required by Capacitor 8 iOS projects. Install from a local path or git URL, then sync:

```sh
npm install ../cordova-plugin-lottie-splashscreen   # or your local path
npx cap sync
```

Configure Lottie preferences in `capacitor.config.ts` (Capacitor does not read Cordova `config.xml` preferences automatically):
...
Place your Lottie JSON under `public/` (or another folder copied into the web build) so it ends up in the native app bundle. On iOS with SPM, the plugin resolves `lottie-ios` via Swift Package Manager; on Android it continues to use Gradle.

Hide the splash from app code once your UI is ready:
...
This plugin does **not** ship its own Android Gradle Plugin (AGP) or Gradle wrapper. Cordova/Capacitor plugins compile inside your app’s native project, so AGP, Gradle, Kotlin, and `compileSdk` are owned by the **host app** (for example `android/build.gradle`, `android/gradle/wrapper/gradle-wrapper.properties`, and `android/variables.gradle` in a Capacitor project).

The plugin only declares its own native **library** dependencies:

| File            | What it pins                                                                             |
| --------------- | ---------------------------------------------------------------------------------------- |
| `plugin.xml`    | Android: Lottie 6.x, AndroidX artifacts                                                  |
| `Package.swift` | iOS: cordova-ios 8.x (rewritten to `capacitor-swift-pm` on `cap sync`), lottie-spm 4.6.x |

Tested against Capacitor 8 apps using AGP **8.13+**, Gradle **8.14+**, `compileSdk` **36**, and Kotlin **2.2+**. Keep your app on those Capacitor 8 defaults rather than duplicating versions inside this plugin repo.
...
npm run lint           # ESLint only (no native CLI tools required)
npm run lint:all       # ESLint + ktlint + swiftlint (requires brew installs below)
npm run setup          # one-time Husky git hooks, optional
```

Native linters are optional and not installed via npm. Install them once if you run `lint:all`:
